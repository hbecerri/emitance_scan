...
if __name__ == "__main__":
    ...
    dets = ["BCM1F"]
    fits = ["SG","DG"] # "SGConst", "DG", "SG","twoMuDG"]
    corrs = ["Background"]
    #corrs = ["noCorr"]#,"BeamBeam_DynamicBeta"]
    energies = ["13600"]

    #my_scan_labels = ['VdM1','VdM2','BI1','VdM3','VdM5','VdM6','VdM7','BI3']
        ...
        for corr in corrs:
            for fit in fits:
                for energy in energies:
                    ...
                    fills_13TeV = [x for x in fills if not x in blacklist_fill]
                    ...
                    if len(sigma_vis) > 0:
                        ...
                        if wantSigVisVsScan:
                            ...
                            i=0
                            height = 132
                            if det == "HFOC": height= 910
                            if det == "HFET": height = 3250
                            if det == "PLT": height = 310
                            if det == "BCM1FUTCA": height = 95
                            for x in fillcounter:
                                print(x)
                                xpos = x[0]-0.4
                                plt.text(xpos, height, f'FILL {fillcounter[i][1]}', horizontalalignment='left', verticalalignment='center',rotation='vertical',size=8,zorder=7, color='purple')
                                i+=1
